...
There are 19 page tools because there are 19 actions. To add one, write `lib/actions/page/<name>.ts` and add it to the array in `lib/actions/registry.ts` — that single edit publishes it as an MCP tool, because the daemon bundles the same registry. See `CLAUDE.md` for the load-bearing conventions (no top-level DOM, `.describe()` every field, no `.refine()`/`.transform()`, no underscores in action names). Verify with `yarn mcp:manifest`.
...
- **Recordings are the reverse of site maps.** The panel's Record button captures what the *user* does in one pinned tab — clicks, fills, navigations — for at most 15 minutes, then a one-shot `claude -p` splits the trace into ordered steps. They live in the extension's storage, not on disk, so `voicelink-mcp skills` does not show them; `page_listRecordings` and `page_readRecording` are how anything reads one, and the `browse-navigation` skill routes on "do it like last time". Typed values are placeholders (`{{email}}`) unless the user ticked "Save what I type", and passwords and card numbers are dropped either way. A recording is a plan, not a script: replaying one still re-verifies each target on the live page, and `page.submitForm` inside a recording is still approval-gated.
