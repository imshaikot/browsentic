...
- **Skills live in `mcp/skills/*.md`** (user overrides in `~/.voicelink/skills/`, hot-reloaded per run). Front-matter routes by trigger words; `@name` prefix forces one. When asked to change how the in-extension agent behaves, that markdown — not this file — is usually the place.
