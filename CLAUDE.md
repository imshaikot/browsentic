...
- **The wire protocol is v5** (`SOCKET_PROTOCOL_VERSION`): on top of the v1 request/reply frames, the extension may send `instruct`/`cancel`/`decision`/`reset`/`analyzeFile`/`saveSkill`/`deleteSkill`/`deleteSiteMap`/`activateSiteMap`/`discardSiteMap` unprompted, and the daemon streams `run` and `siteMapDraft` frames back. Any new extension-initiated frame must also be listed in `EXTENSION_REQUEST_FRAMES` — `isExtensionRequest()` keys on it, and a frame missing from that list is silently dropped by `ExtensionLink.receive` rather than erroring. `assertToolNamesRoundTrip` aside, protocol bumps require rebuilding both sides *and* reloading the extension — an older worker is refused at `hello`.
...
- **Chat responses:** real — the side panel's composer sends instructions through the agent harness (see above); replies and tool activity stream back into `RunTimeline`. Quick commands never get that far: the local intent funnel answers them in the browser (see above).
