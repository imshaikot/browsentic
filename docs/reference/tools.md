...
**Both are side-panel only.** An [MCP client](../guide/mcp-clients.md) cannot install code — it has
...
Gated by the [`code-injection`](../guide/approvals.md) rule, which is why an external MCP client —
...
[`external-code-execution`](../guide/approvals.md).
