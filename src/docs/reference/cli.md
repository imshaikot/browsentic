...
description: "With no command it prints usage. Most commands start the daemon if one is not already running; status, stop, logs, token, tools, skills and approvals do not."
deck: ""
...
| `--browser <name>` | `chrome` only for now. See [install](/docs/guide/install/) for the Firefox situation |
| `--json` | Machine-readable result |

See [guide/install.md](/docs/guide/install/).
...
| Command | Does |
| --- | --- |
| `browsentic mcp` | Serve MCP over stdio. What an [MCP client](/docs/guide/mcp-clients/) runs, not something you type |
