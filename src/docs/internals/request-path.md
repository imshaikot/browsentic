...
pageKey: docs
title: "Path A — an MCP client drives the browser"
seoTitle: "Path A — an MCP client drives the browser — Browsentic internals"
description: "The unconditional path. No agent on Browsentic's side, no intent funnel. The same sequence, animated →"
deck: "The unconditional path. No agent on Browsentic's side, no intent funnel."
docsPath: "internals/request-path.md"
section: "internals"
sectionLabel: "Internals"
sectionOrder: 4
order: 3
isIndex: false
permalink: "/docs/internals/request-path/"
sourceUrl: "https://github.com/imshaikot/browsentic/blob/main/docs/internals/request-path.md"
---
![Path A: an MCP client's tool call reaching the page and the result coming back fenced](/docs/assets/request-path.png)

[The same sequence, animated →](/docs/assets/request-path.gif)
...
default because there is nobody to ask. See [Guardrails](/docs/internals/guardrails/).
...
**[Inside the extension →](/docs/internals/extension/)** — what happens after the invoke frame arrives.
