...
pageKey: docs
title: "Driving Browsentic from an MCP client"
seoTitle: "Driving Browsentic from an MCP client — Browsentic user guide"
description: "Pairing connects your browser to the daemon. Registering an MCP client lets that client drive the same browser — Claude Code, Codex, Cursor, Zed, Gemini CLI…"
deck: "Pairing connects your browser to the daemon. Registering an MCP client lets that client drive the same browser — Claude Code, Codex, Cursor, Zed, Gemini CLI, Claude Desktop, anything that speaks MCP."
docsPath: "guide/mcp-clients.md"
section: "guide"
sectionLabel: "User guide"
sectionOrder: 1
order: 4
isIndex: false
permalink: "/docs/guide/mcp-clients/"
sourceUrl: "https://github.com/imshaikot/browsentic/blob/main/docs/guide/mcp-clients.md"
---
This direction is fully agent-agnostic. Nothing here depends on which CLI you picked in
[Choosing an agent](/docs/guide/agents/).
...
- **41 page tools** — every one listed with its parameters in [reference/tools.md](/docs/reference/tools/)
...
watching. So anything the [policy](/docs/guide/approvals/) would `confirm` resolves to **deny** for an external
...
Understand what that turns back on before you set it — [Approvals](/docs/guide/approvals/) lists every rule,
and [internals/guardrails.md](/docs/internals/guardrails/) explains why the default is the way round
it is.
...
clients get it too. It is not a proof against prompt injection — see [Limits](/docs/guide/limits/#prompt-injection-is-a-real-risk).
...
- [reference/tools.md](/docs/reference/tools/) — every tool and parameter
- [Approvals](/docs/guide/approvals/) — the full policy
- [Troubleshooting](/docs/guide/troubleshooting/) — including "tools missing from an MCP session"
