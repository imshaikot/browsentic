...
pageKey: docs
title: "Configuration"
seoTitle: "Configuration — Browsentic user guide"
description: "Everything is optional. Browsentic works with no config file at all. Settings live in ~/.browsentic/config.json. The daemon re-reads it before every run, so…"
deck: "Everything is optional. Browsentic works with no config file at all."
docsPath: "guide/configuration.md"
section: "guide"
...
sourceUrl: "https://github.com/imshaikot/browsentic/blob/main/docs/guide/configuration.md"
---
...
`~/.browsentic`. See [internals/state.md](/docs/internals/state/) for the full disk layout.
...
Full detail, including the Antigravity permission rule: [Choosing an agent](/docs/guide/agents/).
...
[Conversations](/docs/guide/features/conversations/).
...
| `guardrails.unattended` | `deny` | What a `confirm` becomes for a caller with nobody to ask — i.e. an [MCP client](/docs/guide/mcp-clients/) |
...
Every rule id and what it does, plus the three that the panel shows but will not change: [Approvals](/docs/guide/approvals/).
...
[Site maps](/docs/guide/features/site-maps/).
...
- [Approvals](/docs/guide/approvals/) — the gate, rule by rule
- [internals/state.md](/docs/internals/state/) — every file Browsentic writes
- [reference/cli.md](/docs/reference/cli/) — commands that read and write this config
